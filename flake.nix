{
  ...
  let
    ...
    # Priority: 1. Decrypted metadata.json, 2. Environment variables, 3. Valid defaults
    metadataPath = ./metadata.json;
    sopsKeyFile = ./sopskey.txt;

    # Build-time metadata decryption package
    decryptedMetadata = pkgs.runCommand "decrypt-metadata" {
      nativeBuildInputs = [ pkgs.sops pkgs.jq ];
    } ''
      mkdir -p $out

      # Check if metadata.json exists
      if [ ! -f ${metadataPath} ]; then
        echo "INFO: metadata.json not found, using defaults" >&2
        echo '{}' > $out/metadata.json
        exit 0
      fi

      # Check if sopskey.txt exists
      if [ ! -f ${sopsKeyFile} ]; then
        echo "WARNING: sopskey.txt not found, cannot decrypt metadata.json" >&2
        echo '{}' > $out/metadata.json
        exit 0
      fi
      ...
      if ${pkgs.sops}/bin/sops -d ${metadataPath} > $out/metadata.json 2>/dev/null; then
        echo "Successfully decrypted metadata.json" >&2
      else
        echo "WARNING: Failed to decrypt metadata.json, using defaults" >&2
        echo '{}' > $out/metadata.json
      fi
    '';

    # Load decrypted metadata or fall back to environment variables/defaults
    loadedMetadata = builtins.fromJSON (builtins.readFile "${decryptedMetadata}/metadata.json");

    metadata =
      let
        # Priority: 1. Decrypted metadata.json, 2. Environment variables, 3. Valid defaults
        getValue = jsonKey: envName: default:
          let
            metadataValue = if loadedMetadata ? ${jsonKey} then loadedMetadata.${jsonKey} else "";
            envValue = builtins.getEnv envName;
          in
            if metadataValue != "" && metadataValue != null then metadataValue
            else if envValue != "" then envValue
            else default;
      in {
        # Use valid defaults that satisfy NixOS validation
        # Runtime secrets are also handled by sops-nix (see configuration.nix)
        robotId = getValue "ROBOT_ID" "ROBOT_ID" "polyflow-robot";
        signalingUrl = getValue "SIGNALING_URL" "SIGNALING_URL" "wss://example.com";
        password = getValue "PASSWORD" "PASSWORD" "changeme";
        githubUser = getValue "GITHUB_USER" "GITHUB_USER" "polyflowrobotics";
        turnServerUrl = getValue "TURN_SERVER_URL" "TURN_SERVER_URL" "turn:example.com";
        turnServerUsername = getValue "TURN_SERVER_USERNAME" "TURN_SERVER_USERNAME" "username";
        turnServerPassword = getValue "TURN_SERVER_PASSWORD" "TURN_SERVER_PASSWORD" "password";
      };
